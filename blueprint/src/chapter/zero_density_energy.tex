...
We found the following estimates with the use of computer-aided proof discovery, which improve on \Cref{hb-energy-bound} in various ranges of $\sigma$. First, by using \Cref{hbt} in place of \Cref{hb-energy-simp} in the proof of the previous theorem, it is possible to obtain an improved additive energy estimate for $\sigma \ge 3/4$. A human-readable proof is contained in the following theorem.
...
\begin{proof} Throughout assume that $3/4 \le \sigma \le 5/6$. Choose
\[
\tau_0 = 8\sigma - 4.
\]
We will show that
...
First, consider \eqref{imphb-zlver-ineq}. Suppose that $(\sigma, \tau, \rho, \rho^*, s)\in \Energy_\zeta$ with $3/4 \le \sigma \le 5/6$ and $2 \le \tau \le \tau_0$. Then, from \Cref{hb-12} we have
\begin{equation}\label{hb-lv-rho-form}
\rho \le 2\tau - 12(\sigma - 1/2).
\end{equation}
Furthermore, by \Cref{huxley-lv} and \Cref{power-lemma} with $k = 2$, one has $\rho \le 2\max(2 - 2\sigma, 4 - 6\sigma + \tau/2)$. However since $\tau \le \tau_0 = 8\sigma - 4$, this simplifies to
\begin{equation}
...
\tau' := \tau/k.
\]
Then, by \Cref{huxley-lv} and $\tau' \ge 4\sigma - 2$ one has
\[
\rho/k \le \max(2 - 2\sigma, 4 - 6\sigma + \tau') = 4 - 6\sigma + \tau'.
\]
Also, from \Cref{huxley-lv} and $\tau \le (4\sigma - 2)(k + 1)$ one has
\[
...
\begin{theorem}\label{ze-guthmaynard-thm2}
For $165/226 \le \sigma \le 42/55$ one has
\[
\A^*(\sigma) \le \max\left(\frac{457 - 546\sigma}{2(61 - 58\sigma)(1 - \sigma)}, \frac{5(18 - 19\sigma)}{2(5\sigma + 3)(1 - \sigma)}\right).
\]
\end{theorem}
\derived
\code{prove_zero_density_energy_12()}
...
\begin{table}[ht]
    ...
    \hline
    $\dfrac{457 - 546\sigma}{2(61 - 58\sigma)(1-\sigma)}$ & $\dfrac{165}{226} \leq \sigma \le \dfrac{5831 + \sqrt{60001}}{8240} = 0.7373\ldots$ & Theorem \ref{ze-guthmaynard-thm2}\\
    \hline
    $\dfrac{5(18 - 19\sigma)}{2(5\sigma + 3)(1 - \sigma)}$ & $\dfrac{5831 + \sqrt{60001}}{8240} \leq \sigma \le \dfrac{42}{55} = 0.7636\ldots$ & Theorem \ref{ze-guthmaynard-thm2}, \ref{imp-energy-bound4}\\
    \hline
