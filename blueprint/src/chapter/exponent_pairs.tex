...
\begin{definition}[Exponent pair]\label{exp-pair-def}\uses{phase-def}
\lean{Expdb.IsExponentPair, Expdb.IsExponentPairBound}
\leanok
...
\begin{lemma}[Duality between exponent pairs and $\beta$]\label{beta-duality}\uses{beta-def, exp-pair-def}
\lean{Expdb.isExponentPair_iff_forall_exponentSumGrowthExponent_le}
\leanok
Let $(k,\ell)$ be in the triangle \eqref{exp-pair-triangle}.  Then the following are equivalent:
    \begin{itemize}
    \item[(i)] $(k,\ell)$ is an exponent pair.
...
\begin{corollary}[Exponent pairs closed and convex]\label{exp-pair-closed}\uses{exp-pair-def}
\lean{Expdb.exponentPairs, Expdb.isClosed_exponentPairs, Expdb.convex_exponentPairs}
\leanok
...
\begin{proposition}[Trivial exponent pairs]\label{exp-pair-trivial}\uses{exp-pair-def}
\lean{Expdb.isExponentPair_zero_one, Expdb.isExponentPair_half_half}
$(0,1)$ and $(1/2,1/2)$ are exponent pairs.  (In Lean, the pair $(0,1)$ is proved; the pair $(1/2,1/2)$ is reduced, in \texttt{Expdb.isExponentPair\_half\_half\_of\_exponentSumGrowthExponent\_le\_half}, to the bound $\beta(\alpha) \leq 1/2$ for $1/2 \leq \alpha \leq 1$, which awaits the van der Corput $B$-process.)
\end{proposition}
...
\begin{proposition}[Van der Corput $A$-process]\label{vdc-a}
\lean{Expdb.vanDerCorputA}
...
\begin{proposition}[Van der Corput $B$-process]\label{vdc-b}
\lean{Expdb.vanDerCorputB}
If $(k,\ell)$ is an exponent pair, then so is
...
\begin{theorem}[Cushing (2025) exponent pairs]\label{new-exp-pairs-2}
